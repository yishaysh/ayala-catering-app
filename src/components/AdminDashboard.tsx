    ...
    return (
        <div className="p-8 bg-stone-100 min-h-screen font-sans" dir={language === 'he' ? 'rtl' : 'ltr'}>
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-8">
                ...
                {showAdvancedCalc && (
                    <div className="p-6 bg-stone-50 animate-slide-in-top text-start">
                                ...
                                <table className="w-full border-collapse">
                                    <thead>
                                        <tr className="bg-stone-200 text-stone-600 text-xs uppercase">
                                            ...
                                            <th className="p-3 text-start">{t.tableSandwiches}</th>
                                            <th className="p-3 text-start">{t.tablePastries}</th>
                                            <th className="p-3 text-start">{t.tableSalads}</th>
                                            <th className="p-3 text-start">{t.tableMains}</th>
                                            <th className="p-3 text-start">{t.tablePlatters}</th>
                                            <th className="p-3 text-start">{t.tableDesserts}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {EVENT_TYPES.map(eType => (
                                            <tr key={eType} className="border-b border-stone-200 hover:bg-white">
                                                <td className="p-3 font-bold text-stone-800 capitalize">{rootT[eType] || eType}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-h-[85vh] h-auto md:max-w-lg rounded-2xl p-6 shadow-2xl animate-zoom-in text-start flex flex-col">
                        <div className="flex justify-between items-center mb-6 shrink-0">
                            <h2 className="text-2xl font-serif font-bold text-stone-900">{t.editItemTitle}</h2>
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-h-[90vh] md:max-w-2xl rounded-2xl p-6 shadow-2xl animate-zoom-in text-start flex flex-col">
                        <div className="flex justify-between items-center mb-6 shrink-0"><h2 className="text-2xl font-serif font-bold text-stone-900">{t.createItemTitle}</h2><button onClick={() => setIsAddModalOpen(false)} className="text-stone-400 hover:text-stone-900 bg-stone-100 p-2 rounded-full"><X size={20} /></button></div>
                        <div className="space-y-4 flex-1 overflow-y-auto">
                        ...
                        <div className="mt-8 flex gap-3 shrink-0"><button onClick={handleAddSave} disabled={!newItem.name || !newItem.price} className="flex-1 bg-gold-500 text-stone-900 font-bold py-3 rounded-lg hover:bg-gold-400 disabled:opacity-50 flex items-center justify-center gap-2"><Plus size={18} />{t.create}</button><button onClick={() => setIsAddModalOpen(false)} className="bg-stone-100 text-stone-600 font-bold py-3 px-6 rounded-lg hover:bg-stone-200">{t.cancelBtn}</button></div>
                    </div>
                </div>
            )}
        </div>
