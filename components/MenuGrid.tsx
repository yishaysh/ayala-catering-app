...
import { Plus, Info, Star, Eye, X, Minus, Check } from 'lucide-react';
  ...
  return (
    ...
    <div className="space-y-16 pb-32">
      {CATEGORY_ORDER.map((cat) => {
        ...
        return (
          // Fixed Scroll Margin: Increased to scroll-mt-48 (192px) to clear the sticky header
              ...
              {catItems.map((item) => {
                ...
                return (
                  <div 
                      ...
                      `}
                  >
                    {/* Premium Badge - Moved to Top Right to avoid collision with Price/Name */}
                    {item.is_premium && (
                    ...
                    {/* Image Preview Button - Moved to Top Left */}
                    <div className="absolute top-4 left-4 z-10">
                        <button 
                            ...
                            className="bg-white/90 p-2 rounded-full shadow-md text-stone-600 hover:text-gold-600 transition-colors backdrop-blur-sm border border-stone-100"
                            title="הצג תמונה"
                        >
                            <Eye size={18} />
                        </button>
                    ...
                    <div className="p-6 pt-16"> {/* Increased top padding to avoid overlap with Eye button */}
                    ...
                    <div className="p-4 bg-stone-50 border-t border-stone-100 mt-auto">
                      ...
                      >
                        {!item.availability_status ? t.outOfStock : (
                            <>
                                <Plus size={18} />
                                {guestCount > 0 ? `${t.add} (${suggestedQty})` : t.addToCart}
                            </>
                        )}
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl overflow-hidden shadow-2xl max-w-2xl w-full animate-in zoom-in-95 duration-200">
                <button 
                    ...
                    className="absolute top-4 right-4 z-10 bg-white/50 hover:bg-white text-stone-900 p-2 rounded-full transition-all"
                >
                    <X size={24} />
                </button>
    ...
    {itemToAdd && (
        <div className="fixed inset-0 z-[70] flex items-end md:items-center justify-center p-0 md:p-4 font-sans">
             <div className="absolute inset-0 bg-stone-900/80 backdrop-blur-sm transition-opacity" onClick={() => setItemToAdd(null)}></div>
             <div className="relative bg-white w-full md:max-w-lg md:rounded-2xl rounded-t-2xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom-10 duration-300">
                ...
                <div className="bg-stone-900 p-6 text-white flex justify-between items-start">
                    ...
                    <button onClick={() => setItemToAdd(null)} className="text-stone-400 hover:text-white">
                        <X />
                    </button>
                </div>
                    ...
                    <div className="flex items-center justify-between bg-stone-50 p-4 rounded-xl border border-stone-200">
                        ...
                        <div className="flex items-center gap-3">
                            <button 
                                ...
                                className="w-10 h-10 rounded-full bg-white border border-stone-300 flex items-center justify-center text-stone-600 hover:bg-stone-100"
                            >
                                <Minus size={18} />
                            </button>
                            <span className="text-2xl font-bold w-8 text-center">{addQuantity}</span>
                            <button 
                                onClick={() => setAddQuantity(addQuantity + 1)}
                                className="w-10 h-10 rounded-full bg-gold-500 text-stone-900 flex items-center justify-center shadow-lg hover:bg-gold-400"
                            >
                                <Plus size={18} />
                            </button>
                        </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {getLocalizedItem(itemToAdd, language).modifications.map(mod => (
                                    <button
                                        ...
                                        className={`
                                            px-3 py-1.5 rounded-lg text-sm font-medium border transition-all flex items-center gap-1
                ...
                {/* Footer Actions */}
                <div className="p-6 border-t border-stone-200 bg-stone-50 flex gap-4">
                    ...
                    <button 
                        ...
                        className="flex-[2] bg-gold-500 text-stone-900 font-bold py-3 rounded-xl hover:bg-gold-400 transition-colors shadow-lg"
                    >
                        {t.confirmAdd}
                    </button>
                </div>
